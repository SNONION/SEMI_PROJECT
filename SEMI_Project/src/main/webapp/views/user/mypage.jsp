...
<body>
	...
	<div class="outer">
		...
		<h3 align="center">Item inventory status</h3>
		<br>
			...
			<table class="table table-dark table-striped">
				<thead>
					<tr align="center">
						<th>ITEM CODE</th>
						<th>ITEM NAME</th>
						<th>SELLING ITEM COUNT</th>
					</tr>
				</thead>
				<tbody>
				<c:forEach var="y" items="${mList}">
						<tr align="center">
							<td>${y.proNo}</td>
							<td>${y.proName}</td>
							<td>${y.proCount}</td>
		...
		<div class="modal" id="outputRModal">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body" align="center">
						...
						<div class="adminReply-area">
							<table border="1">
								...
								<script>
								    const currentUserId = "admin01";
								    const allowedUserId = "admin01";
								
								   
								    if (currentUserId === allowedUserId) {
								    	document.getElementById("adminReply-area").innerText = currentUserId;
								        document.getElementById("adminReplyWriter").innerText = currentUserId;
								        document.getElementById("adminReplyDate").innerText = currentUserId;
								        
								    } else {
								        document.getElementById("adminReplyWriter").innerText = "작성 권한이 없습니다.";
								        document.getElementById("adminReplyWriter").style.color = "gray";
								        document.getElementById("adminReplyDate").style.color = "gray";
								    }
								</script>
								<tr>
								...
								<script>
								    ...
								    // 날짜 및 시간을 adminReplyDate 요소에 삽입
								    document.getElementById("adminReplyDate").innerText = formattedDateTime;
								</script>
								<tr>
					...
					<c:choose>
						<c:when test="${loginUser.useId == 'admin01'}">
							<button type="button" onclick="AnswerRequest();" class="btn btn-danger">답변 달기</button>
